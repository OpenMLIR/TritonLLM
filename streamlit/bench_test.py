...
def run(container):
    new_chat_start = time.perf_counter()
    tools = []
    if use_functions:
        tools.append(
            {
    ...
    for line in response.iter_lines(decode_unicode=True):
        ...
        if event_type == "response.output_item.added":
            ...
            reasoning_data = data.get("delta", "")
            if reasoning_data != "":
                if is_first:
                    elapsed = time.perf_counter() - new_chat_start
                    first_token = reasoning_data
                    print(f'TTFT (Time to First Token) : {elapsed:.3f} seconds')
                    token_begin = time.perf_counter()
                is_first = False
            # if data.get("delta", ""):
            #     print(datetime.now(), 'COT ing', data.get("delta", ""))
            output.avatar = "🤔"
            text_delta += reasoning_data
            placeholder.markdown(text_delta)
        elif event_type == "response.output_text.delta":
            # if data.get("delta", ""):
            #     print(datetime.now(), 'output ing', data.get("delta", ""))
            text_delta += data.get("delta", "")
            placeholder.markdown(text_delta)
        elif event_type == "response.output_item.done":
            # print(datetime.now(), 'output end')
            all_text += text_delta
