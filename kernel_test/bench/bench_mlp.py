...
import torch
import argparse
import tritonllm.triton_kernels
...
class PerfData:
    ...
    @property
    def max_tbps(self):
        # https://github.com/triton-lang/triton/blob/v3.4.0/third_party/proton/proton/specs.py#L45
...
def roofline_mlp(batch_ranges, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP=1, EP=1, name="",
    ...
    for batch in batches:
        ...
        if verbose:
            print(f"Batch: {batch}; Util: {perfs[-1].util:8.3f}; TFLOPS: {perfs[-1].tflops:8.3f}; TBPS: {perfs[-1].tbps:8.3f}")
    print("===============================================================")
    # machine limits
